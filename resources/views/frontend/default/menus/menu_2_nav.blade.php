<div class="uk-position-relative">
        <ul {{ $_item->style_id ? "id=\"{$_item->style_id}\"" : NULL }} class="uk-navbar-nav nav-category {{ $_item->style_class }}">
            @foreach($_item->menu_items as $_menu_item)
                @include('frontend.default.menus.menu_2_nav_item', ['_item' => $_menu_item])
            @endforeach
        </ul>

    @if(isset($_accessEdit['menu']) && $_accessEdit['menu'])
        <div class="uk-position-absolute uk-position-top-right uk-margin-small-top">
